...
//                }
    ...
    public function get_order_list(Request $request)
    {
        ...
        $orders->map(function($data) {
            $now = time();
            $your_date = strtotime($data['delivery_date']);
            $datediff = $now - $your_date;
    
            $order_days = round($datediff / (60 * 60 * 24));
            if ($order_days < 7) {
              $data['is_returnable'] = true;
            }else {
        ...
        if ($details->count() > 0) {
            foreach ($details as $det) {
                ...
                if (Order::find($request->order_id)->order_type == 'pos') {
                    ...
                    $det['variation'] = (string) isset($det['variation'][0]['type'])? $det['variation'][0]['type'] :null;
                }
    ...
    public function update_payment_method(Request $request)
    {
      ...
      public function returned_order_initiate(Request $request) {
        ...
        // $order_data = Order::where(['id' => $request['order_id'],'order_status' => 'confirmed', 'payment_status' => 'paid'])->first();
         $order_data = Order::where(['id' => $request['order_id'],'payment_status' => 'paid'])->first();
        //  dd($order_data);
       
        
        $now = time();
        $your_date = strtotime($order_data['delivery_date']);
        $datediff = $now - $your_date;

        $order_days = round($datediff / (60 * 60 * 24));
        if ($order_days < 7) {
            //  Order::where(['id' => $request['order_id'],'order_status' => 'confirmed', 'payment_status' => 'paid'])->update([
            Order::where(['id' => $request['order_id'], 'payment_status' => 'paid'])->update([
                'order_status' => 'returned_order_initiate',"return_reason" => $request['return_reason'],
            ]);
            return response()->json(['code' => 200, 'message' => 'Return Order updated Successfully'], 200);
        }
        return response()->json([
            'errors' => [
                ['code' => 401, 'message' => 'The Order is now out dated to return'],
            ],
        ], 200);
    }
    
     public function returned_order($id) {

        $returned_order =  Order::where('user_id',$id)->where(function($query) {
                            $query->where('order_status','returned')
                                        ->orWhere('order_status','returned_order_initiate');
                                      })->get();
        
        return response()->json([
                'message' => 'Return order got successfully!',
                'order_data' => $returned_order,
            ], 200);
    }
}
